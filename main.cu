...
__device__ vec3 color(const ray& r, hitable** world, curandState* local_rand_state) {
	...
	for (int i = 0; i < 50; i++) {
		...
		if ((*world)->hit(cur_ray, 0.001f, FLT_MAX, si)) {
			ray scattered;
			vec3 attenuation;
			if (si.m->scatter(cur_ray, si, attenuation, scattered, local_rand_state)) {
...
__global__ void render(vec3* fb, int max_x, int max_y, int spp, camera** cam, hitable** world, curandState* rand_state) {
	int i = threadIdx.x + blockIdx.x * blockDim.x;
	int j = threadIdx.y + blockIdx.y * blockDim.y;
	if ((i >= max_x) || (j >= max_y)) return;
	int pixel_index = j * max_x + i;
...
__global__ void create_world(hitable** d_list, hitable** d_world, camera** d_camera, int w, int h, curandState* rand_state) {
	if (threadIdx.x == 0 && blockIdx.x == 0) {
...
int main() {
	...
	std::cerr << "Rendering a " << w << "x" << h << " image with " << spp << " samples per pixel ";
	...
	CHECK_CUDA_ERROR(cudaMalloc((void**)&d_rand_state2, 1 * sizeof(curandState)));
	...
	CHECK_CUDA_ERROR(cudaMalloc((void**)&d_list, num_hitables * sizeof(hitable*)));
	hitable** d_world;
	CHECK_CUDA_ERROR(cudaMalloc((void**)&d_world, sizeof(hitable*)));
	camera** d_camera;
	CHECK_CUDA_ERROR(cudaMalloc((void**)&d_camera, sizeof(camera*)));
	create_world << <1, 1 >> > (d_list, d_world, d_camera, w, h, d_rand_state2);
	CHECK_CUDA_ERROR(cudaGetLastError());
	CHECK_CUDA_ERROR(cudaDeviceSynchronize());
	...
	CHECK_CUDA_ERROR(cudaFree(d_rand_state2));
	CHECK_CUDA_ERROR(cudaFree(fb));

	// Write image to file
	stbi_write_png("output.png", w, h, 3, image, 0);
	delete[] image;
